...
$query = $conexao->prepare("SELECT r.id, p.nome AS paciente_nome, r.nome_medicamento, r.data_administracao, r.hora_administracao, p.leito, r.status FROM receitas r JOIN pessoas p ON r.paciente_id = p.id");
$query->execute();
                        ...
                        <li class="list-group-item d-flex justify-content-between align-items-center col-6 ">
                            <div>
                                <div>
                                    ...
                                    <strong>Data:</strong> <?= htmlspecialchars($receita['data_administracao']) ?>
                                </div>
                            </div>
                            <div>
                                <strong>Hora:</strong> <?= htmlspecialchars($receita['hora_administracao']) ?><br>
                                <strong>Leito:</strong> <?= htmlspecialchars($receita['leito']) ?><br>
                                <strong>Status:</strong> <?= htmlspecialchars($receita['status']) ?>
                            </div>
                            <?php
                            if (!isset($_SESSION['usuario_id']) || $_SESSION['tipo'] == 'enfermeiro' || $_SESSION['tipo'] == 'administrador') { ?>
