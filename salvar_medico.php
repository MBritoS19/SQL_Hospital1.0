...
$comando = $conexao->prepare("SELECT id FROM tipos_pessoa WHERE tipo = 'medico'");
$comando->execute();
$linha = $comando->fetch();
$tipo_id = $linha['id'];

// Inserir dados na tabela `pessoas`
$query = $conexao->prepare("INSERT INTO pessoas (nome, especialidade, crm, tipo_id) VALUES (?, ?, ?, ?)");
$query->execute([$nome, $especialidade, $crm, $tipo_id]);
